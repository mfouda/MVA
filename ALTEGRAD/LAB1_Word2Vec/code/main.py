...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    factors_pos = 1/(np.exp(prodpos)+1).reshape(-1,1)  ## (n_pos,1)
    factors_negs = 1/(np.exp(-prodnegs)+1).reshape(-1,1) ## (n_neg,1)
    
    partials_pos = factors_pos @ -Wt[target,].reshape(1,-1) 
    partials_negs = factors_negs @ Wt[target,].reshape(1,-1)
    
    term_pos  = np.diag(factors_pos.reshape(-1,)) @ -Wc[pos,] ### multiply each row by it's associated factor 
    term_negs = np.diag(factors_negs.reshape(-1,)) @ Wc[negs,]
...
path_read = '/home/mahdo/Desktop/ALTEGRAD_LAB#1/data/'
path_write ='/home/mahdo/Desktop/ALTEGRAD_LAB#1/data/'
...
if not train:
    ...
    print("Simalarity between movie / banana",my_cos_similarity("movie","banana"))
    print("Simalarity between movie / hero ",my_cos_similarity("movie","hero"))
    print("Simalarity between movie / film ",my_cos_similarity("movie","film"))
    
    print("Simalarity between beautiful / attractive ",my_cos_similarity("beautiful","attractive"))
    print("Simalarity between girl / boy",my_cos_similarity("girl","boy"))
